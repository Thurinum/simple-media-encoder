...
void MainWindow::SetupMenu()
{
	...
	menu->addAction(tr("About"), [this]() {
		Notify(Severity::Info, "About " + QApplication::applicationName(), settings.get("Main/sAbout").toString());
	});
...
void MainWindow::ParseContainers(QHash<QString, Container> *containers, QComboBox *comboBox)
{
	...
	for (const QString &containerName : keys) {
		...
		QStringList supportedCodecs =
			settings.get("Containers/" + containerName).toString().split(QRegularExpression("(\\s*),(\\s*)"));
